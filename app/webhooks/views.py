...
def _verify_hmac_signature(secret: str, signature: str, raw_bytes: bytes) -> bool:
    """
    ...
    if not signature or not secret:
        return False
    computed = hmac.new(secret.encode("utf-8"), raw_bytes, hashlib.sha256).hexdigest()
...
def ach_webhook():
    ...
    try:
        ...
        return jsonify({"status": "error", "code": "E_WEBHOOK_INVALID_PAYLOAD"}), 400
    ...
    card = BorrowerCard.query.get(card_id)
    if not card or str(card.user_id) != str(borrower_id):
        return jsonify({"status": "error", "code": "E_WEBHOOK_TENANT_VIOLATION"}), 400
...
def reconcile_webhook():
    ...
    txn = VaultTransaction.query.get(txn_id)
    if not txn:
        return jsonify({
            "status": "error",
