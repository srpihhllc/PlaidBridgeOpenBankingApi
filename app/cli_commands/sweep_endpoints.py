...
@click.option("--filter", "-f", "filter_str", help="Filter endpoints by substring")
@click.option(
    "--json",
    "as_json",
    is_flag=True,
...
def sweep_endpoints(
    ...
    """
    rules = sorted(current_app.url_map.iter_rules(), key=lambda r: r.endpoint or "")
    if filter_str:
    ...
    if as_json:
        data = []
        for r in rules:
            bp = r.endpoint.split(".")[0] if r.endpoint and "." in r.endpoint else "root"
            data.append(
                {
                    "endpoint": r.endpoint,
                    "rule": str(r),
        ...
        click.echo(
            json.dumps(
                {"total_rules": total_rules, "unique_endpoints": unique_endpoints, "rules": data},
                indent=2,
            )
        )
        return
    ...
    if no_group:
        for r in rules:
            methods = ",".join(sorted(r.methods)) if getattr(r, "methods", None) else ""
            click.echo(f"{(r.endpoint or 'unknown'):40} → {str(r):40}  methods: {methods}")
        click.echo("-" * 80)
        return
    ...
    for bp in sorted(groups.keys()):
        ...
        for r in bp_rules:
            methods = ",".join(sorted(r.methods)) if getattr(r, "methods", None) else ""
            click.echo(f"  {(r.endpoint or 'unknown'):36} → {str(r):40}  methods: {methods}")
        click.echo("")
