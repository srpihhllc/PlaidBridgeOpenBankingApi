...
def email_telemetry_now_command():
    ...
    click.echo(
        "🚀 Forcing manual initialization of the reporting subsystem payload..."
    )

    # Deferred import to ensure app context is fully loaded before execution
    from app.services.reporting_subsystem import (
        execute_scheduled_report_dispatch,
    )

    execute_scheduled_report_dispatch()
    click.echo(
        "✨ Sequence complete. Check backend system logs or mailbox for delivery tokens."
    )
