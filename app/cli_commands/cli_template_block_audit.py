...
from flask.cli import with_appcontext  # ✅ Required for context stack resolution
...
def template_block_audit_command():
    ...
    try:
        ...
        click.echo(f"⚠️ Redis connection failed, falling back to local-only audit: {e}", err=True)
        redis_client = None
    ...
    try:
        ...
        click.echo(f"❌ Template block audit encountered a fatal error: {e}", err=True)
        return
    ...
    click.echo(f"Block definitions found:       {summary.get('block_definitions', 0)}")
    click.echo(f"Missing required blocks:       {summary.get('missing_required_blocks', 0)}")
    click.echo(f"Cross-domain block violations: {summary.get('cross_domain_block_violations', 0)}")
