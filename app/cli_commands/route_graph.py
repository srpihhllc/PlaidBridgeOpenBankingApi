...
@click.option("--output", "-o", type=click.Path(dir_okay=False, writable=True), default="-", help="Output file (default stdout). Use .dot extension to save DOT text.")
@with_appcontext
def route_graph(output: str) -> None:
    ...
    for bp in sorted(groups.keys()):
        lines.append(f'  "bp::{bp}" [label="{bp}", shape=folder, style=filled, fillcolor="#f6f8fa"];')
