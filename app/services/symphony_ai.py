...
class SymphonyAI:
    ...
    def execute_instruction(
        self, instruction: str, subscriber_id: str = None
    ) -> dict:
        """
        ...
        current_app.logger.info(
            f"⚡ [SYMPHONY_AI] Processing: {instruction[:50]}..."
        )
        ...
        try:
            ...
            response = self.client.models.generate_content(
                model=self.model_id, contents=instruction, config=config
            )
            ...
            if response.function_calls:
                ...
                simulator_result = self.simulator.apply(
                    ...
                    or tool_args.get("subscriber_id"),
                )
                ...
                self._log_orchestration_event(
                    subscriber_id, instruction, final_result
                )
                return final_result
    ...
    def _log_orchestration_event(
        self, subscriber_id: str, instruction: str, result: dict
    ):
        """Writes cockpit-grade telemetry into Redis."""
        if not self.redis:
            return

        trace_key = f"symphony_tracer:{datetime.now(timezone.utc).timestamp()}"
        payload = {
