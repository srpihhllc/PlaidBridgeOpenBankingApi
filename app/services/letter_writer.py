...
# Internal Helpers
# -----------------------------------------------------------------------------
def _generate_document(template_name: str, context: dict[str, Any]) -> tuple[str | None, str | None]:
    """
...
def generate_letter_3(log_id: int | str) -> tuple[str | None, str | None]:
    ...
    if not user:
        logger.error(f"User ID {lender_record.user_id} associated with Lender ID {log_id} not found.")
        return None, None
    ...
    context = {
        ...
        "lender_name": getattr(lender_record, "institution_name", None) or "Unknown Lender",
        "lender_address": getattr(lender_record, "mailing_address", None) or "Unknown Address",
        "dispute_details": getattr(lender_record, "dispute_reason", None) or "No specific reason provided.",
        "account_number": getattr(lender_record, "account_number", None) or "N/A",
    }

    return _generate_document("dispute_confirmation_l3.txt", context)


def bundle_all_letters(log_id: int | str, document_type: str = "L3_DISPUTE") -> tuple[str | None, str | None]:
    """
    ...
    logger.warning(f"Document type '{document_type}' not supported for log ID {log_id}.")
...
def render_letter_to_text(letter_obj: Any) -> str:
    ...
    try:
        ...
        if isinstance(letter_obj, dict):
            return "\n".join(f"{str(k)}: {str(v)}" for k, v in letter_obj.items())
