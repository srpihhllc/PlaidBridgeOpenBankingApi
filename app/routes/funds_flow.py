...
def view_funds_flow(user_id):
    ...
    if r:
        ...
        flow_data.append(
            {"date": None, "inbound": None, "outbound": None, "net": None}
        )

    return render_template(
        "funds_flow.html", user_id=user_id, flow_data=flow_data
    )
...
def view_anomalies(user_id):
        ...
        for key in r_keys:
            ...
            for entry in raw_entries:
                try:
                    ...
                    continue
                if (
                    obj.get("borrower_id") == user_id
                    or str(user_id) in key.decode()
                ):
    ...
    return render_template(
        "funds_anomalies.html", acct_id=user_id, anomalies=anomalies
    )
