...
def seed_admin():
    ...
    if user:
        ...
        user.set_password(
            password
        )  # Utilizing native model cryptographic helper
        user.is_admin = True
    ...
    try:
        db.session.commit()
        click.echo(
            f"✅ Successfully seeded and verified admin context: {email}"
        )
    except IntegrityError:
        db.session.rollback()
        click.echo(
            "⚠️ ERROR: Integrity conflict detected during admin seed (concurrent process collision?)."
        )
        sys.exit(1)
