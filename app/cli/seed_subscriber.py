...
def seed_subscriber(email, password, username, interactive):
    ...
    if not email or not password:
        click.echo("❌ ERROR: USER_EMAIL and USER_PASSWORD must be set in the environment.")
        sys.exit(1)
    ...
    if user:
        ...
        institution = BankInstitution.query.filter_by(institution_id="demo-bank-001").first()
        if not institution:
            institution = BankInstitution(
                user_id=user.id, name=bank_name, institution_id="demo-bank-001"
            )

        account = BankAccount.query.filter_by(account_number="0001112223").first()
        if not account:
            account = BankAccount(
                user_id=user.id,
    ...
    try:
        ...
        if not BankInstitution.query.filter_by(institution_id="demo-bank-001").first():
            db.session.add(
                BankInstitution(user_id=user.id, name=bank_name, institution_id="demo-bank-001")
            )
