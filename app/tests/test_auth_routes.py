...
def test_mfa_prompt_redirects(client, app, db_session):
    ...
    # Expect the login POST to redirect to MFA prompt (PRG pattern).
    ...
    # Assert redirect to MFA prompt
    assert resp.status_code in (302, 303)
    assert mfa_prompt_url in resp.headers["Location"]

    # Note: MFA flow end-to-end testing (including following redirects and verifying
    # the prompt renders) requires stable session handling in tests.