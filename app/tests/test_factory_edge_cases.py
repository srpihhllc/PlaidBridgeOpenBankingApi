...
def test_factory_invalid_string_config_fallback(caplog):
    ...
    assert app.config["TESTING"] is False  # Falling back to default sets testing false

    has_warning = any("Failed to load config class" in record.message for record in caplog.records)
    assert has_warning
...
def test_factory_config_from_object_exception_fallback():
    ...
    def side_effect(self, obj):
        ...
        raise RuntimeError("Authoritative database profile configuration connection drop error sim")

    side_effect.called_once = False

    # Patch the from_object method with our stateful conditional driver
    with patch("flask.config.Config.from_object", autospec=True, side_effect=side_effect):
...
def test_safe_status_code_exception():
    """Forces _safe_status_code to intercept a non-integer input and return 500."""
...
def test_blueprint_registration_duplicate_skipping(caplog):
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        assert any("already registered globally; skipping" in r.message for r in caplog.records)
...
def test_diagnostic_crash_handler_execution():
    ...
    def force_crash():
        raise RuntimeError("Deliberate factory diagnostic validation assertion failure")
...
def test_ignite_cortex_bad_request_passthrough():
    ...
    def trigger_cortex_err():
        raise BadRequest("Invalid JSON structure format")

    # Patch the property directly on the class to safely feed the intercept logic without clashing context managers
    with patch("flask.Request.endpoint", new_callable=PropertyMock) as mock_endpoint:
...
def test_route_hygiene_and_duplication_handling():
    ...
    def first_rule(): return "1"

    @app.route("/duplicate_path", methods=["GET"])
    def second_rule(): return "2"

    @app.route("/tmp_path", methods=["GET"])
    def temp_rule(): return "tmp"
...
def test_cleanup_premature_oauth_registrations():
    ...
    def fake_oauth_view(): pass
    fake_oauth_view.__module__ = "app.blueprints.oauth_routes_extended"
...
def test_jwt_user_lookup_operator_intercept():
    ...
    with app.app_context():
        ...
        from app.auth_handlers import user_lookup_callback, load_user
        ...
        assert jwt_res is not None, "JWT lookup loader returned None instead of a SystemOperator instance."
        assert jwt_res.username == "OPERATOR_ADMIN", f"Expected OPERATOR_ADMIN, got {jwt_res.username}"
        ...
        assert session_res is not None, "Session loader returned None instead of a SystemOperator instance."
...
def test_production_sentinel_emergency_fallback(caplog):
    ...
    try:
        ...
        with patch.dict(os.environ, {"FLASK_ENV": "production"}), \
             patch("flask.Flask", side_effect=RuntimeError("Production hardware crash sim")):
        ...
        has_fatal_log = any("FATAL BOOT ERROR" in record.message for record in caplog.records)
        assert has_fatal_log, "Emergency module sentinel did not write FATAL BOOT ERROR to logs."
