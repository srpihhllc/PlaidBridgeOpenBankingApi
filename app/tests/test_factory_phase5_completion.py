...
def _get_jwt_user_lookup_callback(app):
    ...
    jwt_manager = app.extensions.get("jwt") or app.extensions.get(
        "flask-jwt-extended"
    )
    if not jwt_manager:
        raise RuntimeError(
            f"JWT manager not found. Keys: {list(app.extensions.keys())}"
        )
...
def test_jwt_user_identity_loader_passthrough(app):
    jwt_manager = app.extensions.get(
        "flask_jwt_extended"
    ) or app.extensions.get("jwt")
    ...
    if identity_callback:
        ...
        assert (
            identity_callback("TERENCE_CORTEX_PRIME") == "TERENCE_CORTEX_PRIME"
        )
...
def test_jwt_user_lookup_cortex_prime_operator(app):
    ...
    with app.app_context():
        ...
        if "SystemOperator" in _jwt_user_lookup.__globals__:
            originals_globals["SystemOperator"] = _jwt_user_lookup.__globals__[
                "SystemOperator"
            ]
        ...
        for g_name, g_val in list(_jwt_user_lookup.__globals__.items()):
            if (
                g_name in ["models", "auth_handlers", "user", "handlers"]
                or "auth" in g_name
            ):
                if g_val and hasattr(g_val, "SystemOperator"):
                    originals_modules_attrs.append(
                        (
                            g_val,
                            "SystemOperator",
                            getattr(g_val, "SystemOperator"),
                        )
                    )
        ...
        for mod_name in target_modules:
            if mod_name in sys.modules:
                ...
                if hasattr(mod, "SystemOperator"):
                    originals_sys_modules[(mod_name, "SystemOperator")] = (
                        getattr(mod, "SystemOperator")
                    )
        ...
        try:
            ...
            with (
                patch(
                    "app.auth_handlers.SystemOperator",
                    MockSysOpClass,
                    create=True,
                ),
                patch(
                    "app.models.user.SystemOperator",
                    MockSysOpClass,
                    create=True,
                ),
                patch(
                    "app.models.SystemOperator", MockSysOpClass, create=True
                ),
                ...
                assert (
                    res is not None
                ), "Lookup returned None. SystemOperator mock failed to intercept initialization."
                assert (
                    res == mock_sys_op_instance
                    or res.__class__.__name__ == "SystemOperator"
                )

                # 2. Exception path: Simulate Cortex boot failure
                MockSysOpClass.side_effect = RuntimeError(
                    "Cortex boot failure"
                )
            ...
            if (
                "SystemOperator" in _jwt_user_lookup.__globals__
                and "SystemOperator" not in originals_globals
            ):
...
def test_jwt_user_lookup_database_resolution_and_integer_fallbacks(app):
    ...
    with app.app_context():
        with patch("app.extensions.db.session.get") as mock_get:
            ...
            assert (
                _jwt_user_lookup({}, {"sub": "uuid-key-xyz"})
                == "UserObjectFound"
            )
            ...
            assert (
                _jwt_user_lookup({}, {"sub": "45678"})
                == "LegacyUserObjectMatched"
            )

        # Case C: Primary miss, parsing ValueError bypass, exits via direct ORM query
        with (
            patch("app.extensions.db.session.get", return_value=None),
            patch("app.models.user.User.query") as mock_query,
        ):
            mock_query.filter_by.return_value.first.return_value = (
                "OrmFallbackUserObject"
            )
            assert (
                _jwt_user_lookup({}, {"sub": "not-an-int"})
                == "OrmFallbackUserObject"
            )
...
def test_create_app_fallback_object_exceptions():
    ...
    with patch(
        ...
        side_effect=side_effect_loader,
        autospec=True,
    ):
