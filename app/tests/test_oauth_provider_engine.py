...
def test_google_exchange_and_profile():
    config = {"client_id": "g-id", "client_secret": "g-secret", "redirect_uri": "g-uri"}
    ...
    responses.add(
        ...
        json={"email": "google@test.com", "sub": "gsub123", "name": "Google User"},
        status=200
    )
...
def test_microsoft_exchange_and_profile():
    config = {"client_id": "m-id", "client_secret": "m-secret", "redirect_uri": "m-uri"}
    ...
    responses.add(
        ...
        json={"mail": None, "userPrincipalName": "ms@test.com", "id": "msub456", "displayName": "MS User"},
        status=200
    )
    profile = provider.fetch_profile(tokens)
...
def test_apple_profile_signature_verification_flow(mock_from_jwk, mock_jwt_decode, mock_unverified_header):
    ...
    mock_jwt_decode.return_value = {"email": "apple@test.com", "sub": "asub789", "name": "Apple User"}
...
@responses.activate
@patch("jwt.get_unverified_header")
def test_apple_profile_signature_invalid_violates_lockout(mock_unverified_header):
    import jwt
    config = {"client_id": "app-id"}
    provider = OAuthProvider(ProviderName.APPLE, config)
    ...
    with patch("jwt.decode", side_effect=jwt.PyJWTError("Signature verification failed")):
        with pytest.raises(RuntimeError, match="Invalid Apple ID Token signature"):
