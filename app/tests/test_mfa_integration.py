...
#              clean database teardown routines.
# =============================================================================
...
def clear_identity_events():
    """Clear test telemetry stream in Redis prior to test execution."""
    rc = get_redis_client()
    if rc:
        try:
...
def assert_event_with_retry(
    ...
    """Retry event stream assertion to handle asynchronous log emissions."""
...
def _body_contains_tokens(resp, tokens) -> bool:
    """Utility to evaluate normalized response body tokens."""
...
def test_login_with_redis_mfa(client, app):
    ...
    try:
        ...
        with app.app_context():
            MFACode.query.filter_by(user_id=user_id).delete()
...
def test_login_with_totp_mfa(client, app):
    ...
    with app.app_context():
        # Clean up existing test records
        User.query.filter_by(username="totp_user").delete()
        db.session.commit()
