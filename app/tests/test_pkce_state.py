...
def test_pkce_and_state_are_set_on_login(client, app):
    ...
    resp = client.get(f"/oauth/login/{ProviderName.GOOGLE.value}")
    ...
    with client:
        resp2 = client.get(f"/oauth/login/{ProviderName.GOOGLE.value}")
        assert resp2.status_code in (302, 303)
        m = re.search(r"state=([^&]+)", resp2.headers["Location"])
        assert m
        state = m.group(1)
        cb = client.get(f"/oauth/callback/{ProviderName.GOOGLE.value}?code=abc123&state={state}")
...
def test_state_mismatch_returns_400(client, app):
    ...
    resp2 = client.get(f"/oauth/callback/{ProviderName.GOOGLE.value}?code=abc123&state=invalid-state")
