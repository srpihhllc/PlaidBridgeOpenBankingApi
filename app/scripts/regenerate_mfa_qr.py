...
def main() -> None:
    ...
    with app.app_context():
        ...
        if not user:
            logger.error(
                "Database lookup failed: No user row located with ID=%s",
                args.user_id,
            )
            raise SystemExit(2)
        ...
        if has_existing_secret and not args.force:
            ...
            try:
                ...
                is_regenerated = True
                logger.info(
                    "Successfully persisted updated totp_secret mapping for user ID %s",
                    user.id,
                )
            except Exception as exc:
                db.session.rollback()
                logger.exception(
                    "Transactional database persistence failure mapping totp_secret on user %s",
                    user.id,
                )
        ...
        try:
            ...
            logger.exception(
                "Provisioning URI compilation step dropped error matrix for user %s",
                user.id,
            )
        ...
        try:
            ...
            if not b64_stream:
                logger.error(
                    "Core engine returned empty base64 payload matrix for user URI %s",
                    user.id,
                )
                raise SystemExit(5)
        except Exception as exc:
            logger.exception(
                "Vector generation engine faulted during processing cycle for user %s",
                user.id,
            )
