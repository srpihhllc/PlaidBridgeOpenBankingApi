...
def _resolve_export_csv() -> Callable[..., Any]:
    ...
    for mod_name in candidates:
        try:
            ...
            logger.debug("Could not import %s (when resolving export_csv)", mod_name, exc_info=True)

    # fallback stub
    def _missing(*args: Any, **kwargs: Any) -> Any:  # pragma: no cover
        raise ImportError("export_csv is not available from csv_utils or csv modules")
    return _missing
...
try:
    ...
    def notify_authorities(*a: Any, **kw: Any) -> Any:  # pragma: no cover - fallback
        raise ImportError("notify_authorities not available")

try:
    from .loan_utils import analyze_loan_agreement  # type: ignore
except Exception:
    def analyze_loan_agreement(*a: Any, **kw: Any) -> Any:  # pragma: no cover - fallback
...
__all__ = ["export_csv", "notify_authorities", "analyze_loan_agreement", "time_since"]