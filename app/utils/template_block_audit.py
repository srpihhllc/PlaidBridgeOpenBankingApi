...
def audit_template_blocks(redis_client) -> dict:
    ...
    try:
        ...
        # 4. Telemetry
        emit_schema_trace(domain="cli", event="template_block_audit", detail="summary", 
                          value="success", status="ok", ttl=600, client=redis_client, meta=summary)

        if missing_required:
            _logger.error("🚨 Missing required blocks:\n%s", json.dumps(missing_required, indent=2))
        if cross_domain:
        ...
        summary["errors"] += 1
        emit_schema_trace(domain="cli", event="template_block_audit", detail="audit_fail", 
