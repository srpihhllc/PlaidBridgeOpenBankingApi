...
safe_emit: Callable[[Callable[..., Any]], Callable[..., Any]] = (
    _default_safe_emit
)
...
try:
    ...
    if (
        telemetry_mod is not None
        and getattr(telemetry_mod, "__name__", "") != __name__
    ):
        _ttl_emit_impl = getattr(telemetry_mod, "ttl_emit", None)
        ttl_summary = getattr(telemetry_mod, "ttl_summary", ttl_summary)
        emit_boot_trace = getattr(
            telemetry_mod, "emit_boot_trace", emit_boot_trace
        )
        safe_emit = getattr(telemetry_mod, "safe_emit", safe_emit)
        flush_emit_queue = getattr(
            telemetry_mod, "flush_emit_queue", flush_emit_queue
        )
except Exception:
