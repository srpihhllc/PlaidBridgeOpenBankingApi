...
def detect_schema_drift(redis_client=None) -> dict:
    ...
    report = {"drift_detected": False, "drift_details": {}, "models_scanned": 0}

    if not os.path.exists(curr_path) or not os.path.exists(stab_path):
        _logger.warning("Baseline or current manifest missing.")
        return report

    with open(curr_path, "r") as f: current_data = json.load(f)
    with open(stab_path, "r") as f: stable_data = {m["name"]: m for m in json.load(f)}

    for model in current_data:
        if model.get("status") != "active": continue
        report["models_scanned"] += 1
        name = model["name"]

        if name in stable_data and model["schema_hash"] != stable_data[name]["schema_hash"]:
