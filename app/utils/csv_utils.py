...
def _normalize_headers(data: Sequence[Mapping[str, Any]], headers: Optional[Sequence[str]] = None) -> List[str]:
    """
...
def export_csv(
    ...
    if output_path:
        ...
        with p.open("w", encoding=encoding, newline=newline) as fh:
            writer = csv.DictWriter(fh, fieldnames=cols, dialect=dialect, extrasaction=extrasaction)
            if cols:
                writer.writeheader()
            for r in rows:
                writer.writerow({k: ("" if r.get(k) is None else str(r.get(k))) for k in cols})
        return str(p)

    # Write to provided file-like object
    if file is not None:
        writer = csv.DictWriter(file, fieldnames=cols, dialect=dialect, extrasaction=extrasaction)
        if cols:
            writer.writeheader()
        for r in rows:
            writer.writerow({k: ("" if r.get(k) is None else str(r.get(k))) for k in cols})
    ...
    sio = io.StringIO()
    writer = csv.DictWriter(sio, fieldnames=cols, dialect=dialect, extrasaction=extrasaction)
    if cols:
        writer.writeheader()
    for r in rows:
        writer.writerow({k: ("" if r.get(k) is None else str(r.get(k))) for k in cols})
