# /home/srpihhllc/PlaidBridgeOpenBankingApi/manage.py

#!/usr/bin/env python3
"""
manage.py — Supreme Executive‑Grade Operational CLI for PlaidBridgeOpenBankingApi
Combines Flask CLI commands and DevOps orchestrations into a single Unified Cockpit.
"""
...
def _safe_register(module_path: str) -> None:
    ...
    try:
        ...
        print(f"❌ [CLI Registry Failure] Core drift in module '{module_path}': {exc}", file=sys.stderr)
...
def run_shell(command: str, abort_on_fail: bool = True):
    ...
    if abort_on_fail and result.returncode != 0:
        click.secho(f"\n[-] FAILED with exit code {result.returncode}", fg="red", bold=True)
...
def update_locks():
    ...
    run_shell(f"{VENV}/bin/pip-compile requirements.txt --output-file={RUNTIME_LOCK}")
    run_shell(f"{VENV}/bin/pip-compile requirements-dev.txt --output-file={DEV_LOCK}")
    run_shell(f"{VENV}/bin/pip-sync {RUNTIME_LOCK} {DEV_LOCK}")
...
def run_ci():
    ...
    run_shell(f"{VENV}/bin/pytest --cov=app --cov-report=xml --cov-report=term-missing --maxfail=1 --disable-warnings -q")
...
def run_dev():
    ...
    click.secho("[+] Starting Flask Development Server...", fg="cyan", bold=True)
    subprocess.run([f"{VENV}/bin/flask", "run"], env=env)

@cli.command("clean")
def run_clean():
    """Remove venv and caches."""
    run_shell(f"rm -rf {VENV} __pycache__ .pytest_cache .mypy_cache .coverage")
...
@click.option("--route", default="/api/v1/fintech/sandbox/account_snapshot", help="Sandbox route to verify")
@with_appcontext
def verify_sandbox(route: str):
    """Verify sandbox mock endpoints for anti-phishing & compliance schema attributes."""
    from flask import current_app
    click.secho(f"\n[+] Executing schema verification on endpoint: {route}", fg="cyan")
    with current_app.test_client() as client:
        response = client.get(route)
        if response.status_code != 200:
            click.secho(f"[-] FAILED: Route returned HTTP {response.status_code}", fg="red")
            return
        ...
        if "unethical_lending_flag" not in compliance:
            errors.append("Missing 'compliance_audit.unethical_lending_flag' attribute.")
        if "predatory_score" not in compliance:
            errors.append("Missing 'compliance_audit.predatory_score' attribute.")
        ...
        if errors:
            ...
            click.secho("\n[✔] SCHEMA VERIFICATION PASSED: All defensive flags present.", fg="green", bold=True)
...
def verify_routes():
    ...
    if duplicates:
        click.secho("\n[!] DUPLICATE ROUTE REGISTRATIONS DETECTED:", fg="yellow", bold=True)
        for ep1, ep2, rule in duplicates:
            click.echo(f"  - Path '{rule}' registered in '{ep1}' and '{ep2}'")
    else:
        click.secho("\n[✔] ROUTE TREE CLEAN: No duplicate endpoint registrations found.", fg="green", bold=True)
